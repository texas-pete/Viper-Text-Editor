...
from tkinter import *
from tkinter import Tk, ttk, Text, Scrollbar, Menu, messagebox, filedialog, Frame, PhotoImage, simpledialog, Button, Entry
...
class TextWindow(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        go_to_menu.add_command(label="Find Window", underline=1, command=self.go_to_find_window, accelerator="Ctrl+F")
    ...
    def create_text_widget(self, frame, *args, **kwargs):
        ...
        text.bind('<Alt-Up>', self.top_line)
        text.bind('<Alt-Down>', self.bottom_line)
    ...
    def see_line_column(self, line_num, column_num, substring_length, editor_text, find_string):

        # utility for status of current_match / total_matches
        self.current_find_out_of_all(editor_text, find_string, line_num, column_num)

        # still hard to tell difference between 2 or more substrings found in the same line
        column_num = column_num + substring_length
        see_line_position = str(line_num) + "." + str(column_num)
        #print("insert set at: " + see_line_position)
        self.tabs[self.get_tab()].text.see(see_line_position)
        self.tabs[self.get_tab()].text.mark_set('insert', see_line_position)

    def current_find_out_of_all(self, editor_text, find_string, line_num, column_num):
        line_count = 1
        find_count = 0
        total_count = editor_text.count(find_string)
        for line in editor_text.split("\n"):
            # accounts for offset by making a slice
            if line_count <= line_num:
                if line_num == line_count:
                    line2 = line[:column_num]
                    find_count += line2.count(find_string)
                    break
                else:
                    find_count += line.count(find_string)
            line_count += 1
        ...
        self.app2.update_label(find_count, total_count, line_num, column_num)
        
    def top_line(self, event):
        try:
            # set insertion position to inputted line #
            go_to_line_number = 0
            if go_to_line_number is not None:
