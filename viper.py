...
class TextWindow(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        bookmark_menu.add_command(label="Toggle", underline=1, command=self.toggle_bookmark, accelerator= "Alt + Shift + T")
        bookmark_menu.add_command(label="Clear", underline=1, command=self.clear_bookmarks, accelerator= "Alt + Shift + C")
        bookmark_menu.add_command(label="Next", underline=1, command=self.next_bookmark, accelerator= "Alt + Shift + N")
        bookmark_menu.add_command(label="Previous", underline=1, command=self.previous_bookmark, accelerator= "Alt + Shift + P")
    ...
    def create_text_widget(self, frame, *args, **kwargs):
        ...
        text.bind('<Alt-Shift-T>', self.toggle_bookmark)
        text.bind('<Alt-Shift-C>', self.clear_bookmarks)
        text.bind('<Alt-Shift-N>', self.next_bookmark)
        text.bind('<Alt-Shift-P>', self.previous_bookmark)
    ...
    def previous_bookmark(self, event):
        if len(self.tabs[self.get_tab()].get_bookmarks()) > 0:
            insertion_position = self.tabs[self.get_tab()].text.index("insert")
